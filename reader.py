...
def load_pretrain(path, datasets):
    ...
    with open(path, "r") as f:
        ...
        for l in f:
            ...
            if i%1000 == 0:
                sys.stdout.write(".")
        f.close()
    '''for words not in vocabulary, map them to OOV word ' '.'''
    vocab.append(' ')
    emb.append(np.random.uniform(-0.1, 0.1, len(emb[0])))
    ...
    pretrain_emb = np.asarray(emb).astype(np.float)
...
class DataSet:
    def __init__(self, data_path, max_word_len, pretrain_word2id, pretrain_id2word, pretrain_emb, vocabs, keep_origin=False):
        ...
        self.pretrain_emb = pretrain_emb
            #self.word2id, self.id2word, self.char2id, self.id2char, self.tag2id, self.id2tag = bld_vocab(data_path)
    ...
    def load_data(self):
        ...
        for sent in self.sents:
            for p in sent:
                ...
                self.words.append(p[0])
                self.tags.append(p[3])
                self.chars.append(list(self.pad_word(p[0])))

        self.word_array = [self.pretrain_word2id[w] if w in self.pretrain_word2id else self.pretrain_word2id[' '] for w in self.words]
        self.tag_array = [self.tag2id[w] if w in self.tag2id else self.tag2id[' '] for w in self.tags]
        self.char_array = []
